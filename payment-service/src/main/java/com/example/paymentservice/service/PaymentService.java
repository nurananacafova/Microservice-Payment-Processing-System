...
public class PaymentService {
    ...
    public Payment processPayment(PaymentRequestDto dto) {
        ...
        if (dto.getAmount().compareTo(new BigDecimal("3000")) > 0) {
            ...
            kafkaTemplate.send("risk-check-required", riskRequest);
    ...
    private void processPaymentInternal(Payment payment) {
        ...
        if (current == null || current.compareTo(payment.getAmount()) < 0) {
            payment.setStatus(TransactionStatus.FAILED);
            throw new InsufficientFundsException("Insufficient funds for account " + payment.getFromAccount());
        }

        accountServiceClient.updateBalance(payment.getFromAccount(),
                new UpdateBalanceRequest(payment.getCurrency(), payment.getAmount().negate(), payment.getTransactionId()));

        accountServiceClient.updateBalance(payment.getToAccount(),
                new UpdateBalanceRequest(payment.getCurrency(), payment.getAmount(), payment.getTransactionId()));
    ...
    @KafkaListener(topics = "risk-check-approved")
